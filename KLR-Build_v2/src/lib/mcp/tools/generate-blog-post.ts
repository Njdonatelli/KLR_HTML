...
export default defineTool({
  ...
  description:
    "Generate a KLR Build blog article about an exterior or landscaping home improvement topic using Lovable AI. Returns a ~300-400 word article with practical tips, cost guidance, and actionable advice.",
  inputSchema: {
    topic: z.string().describe("The blog post topic, e.g. 'Drought-Tolerant Planting Guides'."),
    type: z.enum(["exterior", "landscaping"]).describe("Whether the topic is exterior or landscaping."),
  },
  ...
  handler: async ({ topic, type }) => {
    ...
    const userPrompt =
      type === "landscaping"
        ? `Write a comprehensive blog post about ${topic} for landscaping. Include practical tips, design ideas, and budget considerations. Make it around 300-400 words.`
